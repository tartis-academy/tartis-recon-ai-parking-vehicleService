...
VEHICLE_DIR="backend/vehicle-service"
...
case "${1:-up}" in
  down|clean)
    ...
    (cd "$VEHICLE_DIR" && docker compose down $FLAG) || true
...
info "Ficheros .env"
for dir in "." "$VEHICLE_DIR"; do
  if [ -f "$dir/.env" ]; then
    ok "$dir/.env ya existe, no lo toco."
  elif [ -f "$dir/.env.example" ]; then
    cp "$dir/.env.example" "$dir/.env"
    ok "$dir/.env creado desde .env.example."
    warn "Lleva contrasenas 'change.me'. Cambialas si esto no es tu portatil."
  else
    die "Falta $dir/.env.example"
  fi
done
